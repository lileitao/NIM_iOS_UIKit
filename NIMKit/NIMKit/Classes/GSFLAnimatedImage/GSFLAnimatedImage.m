...
const NSTimeInterval kGSFLAnimatedImageDelayTimeIntervalMinimum = 0.02;
...
@optional
- (void)debug_animatedImage:(GSFLAnimatedImage *)animatedImage didUpdateCachedFrames:(NSIndexSet *)indexesOfFramesInCache;
- (void)debug_animatedImage:(GSFLAnimatedImage *)animatedImage didRequestCachedFrame:(NSUInteger)index;
- (CGFloat)debug_animatedImagePredrawingSlowdownFactor:(GSFLAnimatedImage *)animatedImage;
@end
#endif


@interface GSFLAnimatedImage ()
...
@property (nonatomic, strong, readonly) GSFLAnimatedImage *weakProxy;
...
@implementation GSFLAnimatedImage
...
+ (void)initialize
{
    if (self == [GSFLAnimatedImage class]) {
...
- (instancetype)init
{
    GSFLAnimatedImage *animatedImage = [self initWithAnimatedGIFData:nil];
    if (!animatedImage) {
        FLLog(GSFLLogLevelError, @"Use `-initWithAnimatedGIFData:` and supply the animated GIF data as an argument to initialize an object of type `FLAnimatedImage`.");
    }
...
- (instancetype)initWithAnimatedGIFData:(NSData *)data optimalFrameCacheSize:(NSUInteger)optimalFrameCacheSize predrawingEnabled:(BOOL)isPredrawingEnabled
{
    ...
    if (!hasData) {
        FLLog(GSFLLogLevelError, @"No animated GIF data supplied.");
        return nil;
    }
    ...
    if (self) {
        ...
        if (!_imageSource) {
            FLLog(GSFLLogLevelError, @"Failed to `CGImageSourceCreateWithData` for animated GIF data %@", data);
            return nil;
        }
        ...
        if (!isGIFData) {
            FLLog(GSFLLogLevelError, @"Supplied data is of type %@ and doesn't seem to be GIF data %@", imageSourceContainerType, data);
            return nil;
        }
            ...
            @autoreleasepool {
                ...
                if (frameImageRef) {
                    ...
                    if (frameImage) {
                        ...
                        if (!delayTime) {
                            if (i == 0) {
                                FLLog(GSFLLogLevelInfo, @"Falling back to default delay time for first frame %@ because none found in GIF properties %@", frameImage, frameProperties);
                                delayTime = @(kDelayTimeIntervalDefault);
                            } else {
                                FLLog(GSFLLogLevelInfo, @"Falling back to preceding delay time for frame %zu %@ because none found in GIF properties %@", i, frameImage, frameProperties);
                        ...
                        if ([delayTime floatValue] < ((float)kGSFLAnimatedImageDelayTimeIntervalMinimum - FLT_EPSILON)) {
                            FLLog(GSFLLogLevelInfo, @"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).", i, [delayTime floatValue], kDelayTimeIntervalDefault, kGSFLAnimatedImageDelayTimeIntervalMinimum);
                            delayTime = @(kDelayTimeIntervalDefault);
                        }
                        delayTimesForIndexesMutable[@(i)] = delayTime;
                    } else {
                        skippedFrameCount++;
                        FLLog(GSFLLogLevelInfo, @"Dropping frame %zu because valid `CGImageRef` %@ did result in `nil`-`UIImage`.", i, frameImageRef);
                    }
                    CFRelease(frameImageRef);
                } else {
                    skippedFrameCount++;
                    FLLog(GSFLLogLevelInfo, @"Dropping frame %zu because failed to `CGImageSourceCreateImageAtIndex` with image source %@", i, _imageSource);
                }
            }
        }
        ...
        if (self.frameCount == 0) {
            FLLog(GSFLLogLevelInfo, @"Failed to create any valid frames for GIF with properties %@", imageProperties);
            return nil;
        } else if (self.frameCount == 1) {
            // Warn when we only have a single frame but return a valid GIF.
            FLLog(GSFLLogLevelInfo, @"Created valid GIF but with only a single frame. Image properties: %@", imageProperties);
        } else {
        ...
        _weakProxy = (id)[GSFLWeakProxy weakProxyForObject:self];
...
{
    GSFLAnimatedImage *animatedImage = [[GSFLAnimatedImage alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
- (UIImage *)imageLazilyCachedAtIndex:(NSUInteger)index
{
    ...
    if (index >= self.frameCount) {
        FLLog(GSFLLogLevelWarn, @"Skipping requested frame %lu beyond bounds (total frame count: %lu) for animated image: %@", (unsigned long)index, (unsigned long)self.frameCount, self);
        return nil;
    }
...
- (void)addFrameIndexesToCache:(NSIndexSet *)frameIndexesToAddToCache
{
    ...
    if (firstRange.length + secondRange.length != self.frameCount) {
        FLLog(GSFLLogLevelWarn, @"Two-part frame cache range doesn't equal full range.");
    }
    ...
    GSFLAnimatedImage * __weak weakSelf = self;
    dispatch_async(self.serialQueue, ^{
        ...
        void (^frameRangeBlock)(NSRange, BOOL *) = ^(NSRange range, BOOL *stop) {
            ...
            for (NSUInteger i = range.location; i < NSMaxRange(range); i++) {
                ...
                FLLog(GSFLLogLevelVerbose, @"Predrew frame %lu in %f ms for animated image: %@", (unsigned long)i, (predrawDuration + slowdownDuration) * 1000, self);
#endif
...
+ (CGSize)sizeForImage:(id)image
{
    ...
    } else if ([image isKindOfClass:[GSFLAnimatedImage class]]) {
        GSFLAnimatedImage *animatedImage = (GSFLAnimatedImage *)image;
        imageSize = animatedImage.size;
    } else {
        // Bear trap to capture bad images; we have seen crashers cropping up on iOS 7.
        FLLog(GSFLLogLevelError, @"`image` isn't of expected types `UIImage` or `FLAnimatedImage`: %@", image);
    }
...
- (NSMutableIndexSet *)frameIndexesToCache
{
    ...
    if (self.frameCacheSizeCurrent == self.frameCount) {
        ...
        if ([indexesToCache count] != self.frameCacheSizeCurrent) {
            FLLog(GSFLLogLevelWarn, @"Number of frames to cache doesn't equal expected cache size.");
        }
...
- (void)growFrameCacheSizeAfterMemoryWarning:(NSNumber *)frameCacheSize
{
    ...
    FLLog(GSFLLogLevelDebug, @"Grew frame cache size max to %lu after memory warning for animated image: %@", (unsigned long)self.frameCacheSizeMaxInternal, self);
...
- (void)resetFrameCacheSizeMaxInternal
{
    ...
    FLLog(GSFLLogLevelDebug, @"Reset frame cache size max (current frame cache size: %lu) for animated image: %@", (unsigned long)self.frameCacheSizeCurrent, self);
}
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification
{
    ...
    FLLog(GSFLLogLevelDebug, @"Attempt setting frame cache size max to %lu (previous was %lu) after memory warning #%lu for animated image: %@", (unsigned long)FLAnimatedImageFrameCacheSizeLowMemory, (unsigned long)self.frameCacheSizeMaxInternal, (unsigned long)self.memoryWarningCount, self);
...
+ (UIImage *)predrawnImageFromImage:(UIImage *)imageToPredraw
{
    ...
    if (!colorSpaceDeviceRGBRef) {
        FLLog(GSFLLogLevelError, @"Failed to `CGColorSpaceCreateDeviceRGB` for image %@", imageToPredraw);
    ...
    // Early return on failure!
    if (!bitmapContextRef) {
        FLLog(GSFLLogLevelError, @"Failed to `CGBitmapContextCreate` with color space %@ and parameters (width: %zu height: %zu bitsPerComponent: %zu bytesPerRow: %zu) for image %@", colorSpaceDeviceRGBRef, width, height, bitsPerComponent, bytesPerRow, imageToPredraw);
        return imageToPredraw;
    }
    ...
    // Early return on failure!
    if (!predrawnImage) {
        FLLog(GSFLLogLevelError, @"Failed to `imageWithCGImage:scale:orientation:` with image ref %@ created with color space %@ and bitmap context %@ and properties and properties (scale: %f orientation: %ld) for image %@", predrawnImageRef, colorSpaceDeviceRGBRef, bitmapContextRef, imageToPredraw.scale, (long)imageToPredraw.imageOrientation, imageToPredraw);
        return imageToPredraw;
    }
...
@implementation GSFLAnimatedImage (Logging)

static void (^_logBlock)(NSString *logString, GSFLLogLevel logLevel) = nil;
static GSFLLogLevel _logLevel;

+ (void)setLogBlock:(void (^)(NSString *logString, GSFLLogLevel logLevel))logBlock logLevel:(GSFLLogLevel)logLevel
{
    _logBlock = logBlock;
    _logLevel = logLevel;
}

+ (void)logStringFromBlock:(NSString *(^)(void))stringBlock withLevel:(GSFLLogLevel)level
{
...
{
    GSFLWeakProxy *weakProxy = [GSFLWeakProxy alloc];
