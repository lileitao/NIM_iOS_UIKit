...
#import "GSFLAnimatedImageView.h"
...
@optional
- (void)debug_animatedImageView:(GSFLAnimatedImageView *)animatedImageView waitingForFrame:(NSUInteger)index duration:(NSTimeInterval)duration;
@end
#endif


@interface GSFLAnimatedImageView ()
...
@implementation GSFLAnimatedImageView
...
- (void)setAnimatedImage:(GSFLAnimatedImage *)animatedImage
{
...
- (NSTimeInterval)frameDelayGreatestCommonDivisor
{
    ...
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kGSFLAnimatedImageDelayTimeIntervalMinimum;
...
- (void)startAnimating
{
    if (self.animatedImage) {
        ...
        if (!self.displayLink) {
            ...
            GSFLWeakProxy *weakProxy = [GSFLWeakProxy weakProxyForObject:self];
...
- (void)displayDidRefresh:(CADisplayLink *)displayLink
{
    ...
    // Early return!
    if (!self.shouldAnimate) {
        FLLog(GSFLLogLevelWarn, @"Trying to animate image when we shouldn't: %@", self);
        return;
    }
    ...
    if (delayTimeNumber) {
        ...
        if (image) {
            FLLog(GSFLLogLevelVerbose, @"Showing frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
            ...
            FLLog(GSFLLogLevelDebug, @"Waiting for frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
#if defined(DEBUG) && DEBUG
