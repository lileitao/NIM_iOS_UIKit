...
extern const NSTimeInterval kGSFLAnimatedImageDelayTimeIntervalMinimum;
...
//
@interface GSFLAnimatedImage : NSObject
...
typedef NS_ENUM(NSUInteger, GSFLLogLevel) {
    GSFLLogLevelNone = 0,
    GSFLLogLevelError,
    GSFLLogLevelWarn,
    GSFLLogLevelInfo,
...
@interface GSFLAnimatedImage (Logging)
...
#define FLLog(logLevel, format, ...) [GSFLAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]

@interface GSFLWeakProxy : NSProxy
